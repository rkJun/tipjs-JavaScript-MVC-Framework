...
(function() {
	...
	var __makeAppRequireFileList__ = function(define, depart) {
		...
		var _appName = define.name, _path = tipJS._config.path[depart], _applicationRoot = tipJS._config.applicationPath[_appName], _departs = __uniqueArray__(define[depart]);
	...
	var __loadJsFile__ = function(file, noCacheOpt, callbackFn) {
		...
		_tagScript.charset = tipJS._config.charSet;
	...
	var __getApp__ = function(appName) {
		return tipJS.__application__[appName];
	};
	...
	var __getAppDefine__ = function(appName) {
		return tipJS.__application__[appName].define;
	};
	...
	var __afterApplicationLoaded__ = function(appName) {
		...
		(function(appName) {
			...
			if (tipJS.__reservedStack__ && tipJS.__reservedStack__[appName]) {
				var _reservedAction = tipJS.__reservedStack__[appName];
				...
				delete tipJS.__reservedStack__[appName];
			};
		})(appName);
	};
	...
	var __wrapPath__ = function(path) {
		return tipJS._const.pathDiv + path + tipJS._const.pathDiv;
	};
	...
	var __loadCommonJSFiles__ = function(config, arrayJS) {
		for (var i = 0, len = arrayJS.length; i < len; i++) {
			...
			if (config.noCache && config.noCache === true) {
				...
				src = src + (config.noCacheParam ? config.noCacheParam : tipJS._config.noCacheParam) + "=";
				if (config.noCacheAuto && config.noCacheAuto === true) {
					src = src + Math.random();
				} else {
					src = src + (config.noCacheVersion ? config.noCacheVersion : tipJS._config.noCacheVersion);
				}

			};
			document.write('<script type="text/javascript" charset="' + (config.charSet ? config.charSet : tipJS._config.charSet) + '" src="' + src + '"></script>');
		};
	};
	 ...
	 * @param config
	 */
	var __config__ = function(config) {
		if (config.commonLib) {
		...
		tipJS._config = __mergeObject__(config, __TIPJS_TEMPLATE__.OBJECT_TEMPLATE.config);
		if (tipJS.isDevelopment === null) {
			var _hostname = window.location.hostname;
			for (var i = 0, _localhostLen = tipJS._config.developmentHostList.length; i < _localhostLen; i++) {
				if (_hostname.match(tipJS._config.developmentHostList[i]) !== null) {
	...
	tipJS.config = __config__;
	tipJS._const = (function() {
		var _const = {
			pathDiv : "/",
			blank : "",
			extJS : "js",
	...
	tipJS.log = function(msg) {
		...
		console.log(_yyyy + '/' + _mm + '/' + _dd + ' ' + _hh + ':' + _mi + ':' + _ss + '.' + _ms + ' ### ' + msg);
	};
	...
	tipJS.action = function(controllerName, params) {
		...
		if (!_application || !_application.loadOrder || !_application.loadOrder.isLastOrder()) {
			tipJS.__reservedStack__ = tipJS.__reservedStack__ || {};
			tipJS.__reservedStack__[_appName] = tipJS.__reservedStack__[_appName] || [];
			tipJS.__reservedStack__[_appName].push({
				name : controllerName,
				param : params
			});
			return;
	...
	tipJS.loadApp = function(appNames, params) {
		for (var i = 0, appLen = appNames.length; i < appLen; i++) {
			...
			if (params) {
				tipJS.__application__[_appName] = tipJS.__application__[_appName] || {};
				tipJS.__application__[_appName].onLoadParam = params;
			};
			_filepath.push(tipJS._config.applicationPath[_appName]);
			_filepath.push(tipJS._const.pathDiv);
			_filepath.push(tipJS._config.defineFileName);
			_filepath.push(tipJS._const.extDiv);
			_filepath.push(tipJS._const.extJS);
			setTimeout(function() {
				if (!tipJS.__application__[_appName].define) {
					throw "Could not find application: " + _appName;
				}
			}, 1000);
			__loadJsFile__(_filepath.join(tipJS._const.blank), {
				nocache : true,
				version : Math.random(),
				paramName : tipJS._config.noCacheParam
			});
		};
	};
	...
	tipJS.define = function(define) {
		...
		if (define.noCache === undefined) {
			define.noCache = tipJS._config.noCache;
			define.noCacheVersion = tipJS._config.noCacheVersion;
			define.noCacheParam = tipJS._config.noCacheParam;
			define.noCacheAuto = tipJS._config.noCacheAuto;
		} else {
		...
		tipJS.__application__[_appName] = tipJS.__application__[_appName] || {};
		tipJS.__application__[_appName].loadOrder = {};
		tipJS.__application__[_appName].loadOrder = __mergeObject__(tipJS.__application__[_appName].loadOrder, __TIPJS_TEMPLATE__.OBJECT_TEMPLATE.loadOrder);

		var _depart = tipJS.__application__[_appName].loadOrder.presentOrder();
		__initAppTemplateNS__(_appName, _depart);
		tipJS.__application__[_appName].define = define;
	...
	tipJS.__application__ = tipJS.__application__ || {};
	tipJS._config = tipJS._config || {};
	tipJS._config = __mergeObject__(tipJS._config, __TIPJS_TEMPLATE__.OBJECT_TEMPLATE.config);
	var _pathname = window.location.pathname, _queryString = window.location.search, _scripts = document.getElementsByTagName('script'), _filepath, _scriptSrc, _match, _localhostLen, _isDevelopment = null, _developmentHostList = tipJS._config.developmentHostList;

	for (var i = 0, scriptLen = _scripts.length; i < scriptLen; i++) {
		_scriptSrc = _scripts[i].src;
		_match = _scriptSrc.match(/tipJS-MVC-1\.19-dev\.js$/);
		if (_match) {
	...
	document.write('<script type="text/javascript" charset="UTF-8" src="' + _filepath + 'tipJS.config.js?' + tipJS._config.noCacheParam + '=' + Math.random() + '"></script>');
