/*
 * tipJS - OpenSource Javascript MVC Framework ver.1.32
 *
...
(function() {
	 ...
	 * @return Object
	 */
	var __getById__ = function(id, obj){
	 ...
	 * @return Objects
	 */
	var __getByName__ = function(name, obj){
	 ...
	 * @return Objects
	 */
	var __getByTag__ = function(tag, obj){
	...
	var __makeAppReqFileList__ = function(define, depart) {
		...
		if (depart === __TIPJS_TPL__.OBJ_TPL.loadOrder.order[1] && define.localSet) {
			define[depart] = [_filepath+__config__.path.lang+__const__.pathDiv+tipJS.lang+".js"];
	...
	var __afterAppLoaded__ = function(appName) {
		...
		if (_ctrlers) {
			for (var k in _ctrlers) {
				...
				_ctrlers[k].getById = __getById__;
				_ctrlers[k].getByName = __getByName__;
				_ctrlers[k].getByTag = __getByTag__;
			}
		}
		(function(appName) {
			var _app = __getApp__(appName);
	 ...
	 * @return Object Clone
	 */
	/*var __cloneObject__ = function(target) {
		function F(){};
		F.prototype = target;
		return new F();
	}*/
	...
	/**
	 * 인수로 들어온 Object 의 복제를 반환(호환성버전)
	 *
	 * @param target
	 * @return Object Clone
	 */
	var __cloneObject__ = tipJS.cloneObject = function(obj){
		...
		for (var k in obj) {
			if (obj[k] && typeof obj[k] == "object") {
				newObj[k] = tipJS.cloneObject(obj[k]);
			} else {
				newObj[k] = obj[k];
			};
		};
	...
	var __echo__ = tipJS.echo = function(target, filter, parentName) {
		...
		for (var k in target) {
			if (target[k] && target[k] instanceof Array) {
				console.log(((parentName) ? parentName + "." : "") + k + ":Array["+ target[k].length + "]");
	...
	var __getTemplate__ = function(config) {
		if (arguments.length > 1) {
			return __renderTemplate__(arguments[0], arguments[1]);
		}
		var _appName = __app__.MAIN,
			_templateKeyArr = config.url.split('#'),
			_templateUrl = _templateKeyArr[0],
			_templateKey = null;
		if (_templateKeyArr.length == 2) {
			_templateKey = _templateKeyArr[1];
		}
		if (__getAppDefine__(_appName).templateCache && __templateCache__[_templateUrl]) {
			var _retTxt = __renderTemplate__(__templateCache__[_templateUrl], config.data, _templateKey);
		...
		var _appNoCacheInfo = __getAppNoCacheInfo__(_appName),
			_fileUrl = __makeNocacheStr__(_templateUrl, _appNoCacheInfo),
		...
		if (_xmlhttp.readyState == 4 && _xmlhttp.status == 200) {
			var _retTxt = __templateCache__[_templateUrl] = _xmlhttp.responseText;
			_retTxt = __renderTemplate__(_retTxt, config.data, _templateKey);
			if (typeof config.renderTo == "string")
				document.getElementById(config.renderTo).innerHTML += _retTxt;
	 ...
	 */
	var __renderTemplate__ = function(html, data, templateKey) {
		html = html.replace(/\r\n/g, "\n");
	...
	tipJS.localSet = function(msgs) {
		var _app = __getApp__(__app__.MAIN);
		if (_app.loadOrder.presentOrder() === __TIPJS_TPL__.OBJ_TPL.loadOrder.order[1]) __msg__ = msgs;
	}
	...
	tipJS.action = function(ctrlerName, params) {
		...
		if (!_app.controller || !_app.controller[_ctrlerName])
			throw "Could not find controller: " + _appName + "." + ctrlerName;
		...
		if (_ctrlerWrapper.beforeCtrler) {
			if (_ctrlerWrapper.beforeCtrler(params) === false)
				return;
		}
		...
		var _doCtrler = function() {
			...
			if (tipJS.isDevelopment === true)
				tipJS.debug(ctrlerName + " completed in " + ((__getSeconds__() - _ctrlerStartTime)/1000) + " seconds");
		} // _doCtrler
	...
	var __app__ = {},
		...
		_pathname = window.location.pathname, _queryString = window.location.search, _scripts = document.getElementsByTagName('script'), _filepath, _scriptSrc, _match, _isDevelopment = null, _lang = (navigator.language || navigator.systemLanguage || navigator.userLanguage).substr(0,2);

	for (var i = _scripts.length; i--;) {
		_scriptSrc = _scripts[i].src;
		_match = _scriptSrc.match(/tipJS-MVC-1\.32-dev\.js$/);
		if (_match) {
